...
function Login({ setToken, setUsername }) {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${API_URL}/login`, { username: usernameInput, password });
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full z-10 mx-4">
      ...
      <h2 className="text-2xl font-normal text-center text-gray-800 mb-8">Log in to WhatsApp Web</h2>
      ...
      <p className="mt-8 text-sm text-center text-gray-500">
        Don't have an account? <Link to="/register" className="text-[#00a884] font-semibold hover:underline">Register here</Link>
      </p>
    </div>
  );
}
